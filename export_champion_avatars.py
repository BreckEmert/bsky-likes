...
Now also called automatically at the end of export_champions.py, so the board is
never left picture-less after a regen. Can still be run standalone:
...
def add_avatars(champions_path=CH):
    """Fetch + inject the {handle: avatarUrl} map into champions.json (in place)."""
    champions_path = Path(champions_path)
    d = json.loads(champions_path.read_text(encoding="utf-8"))
    ...
    d["avatars"] = avatars
    champions_path.write_text(json.dumps(d, ensure_ascii=False), encoding="utf-8")
